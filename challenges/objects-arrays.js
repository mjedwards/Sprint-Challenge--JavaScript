...
lowCaseAnimalNames.push(zooAnimals.map(x => x.animal_name.toLocaleLowerCase()));
...
lowPopulationAnimals.push(zooAnimals.filter(z => z.population < 5));
...
let nums = [];
zooAnimals.forEach(we => {
  nums.push(we.population)
})

populationTotal = nums.reduce((a,b) => a+b);
